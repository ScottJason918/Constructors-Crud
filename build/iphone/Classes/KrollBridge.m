...
NSString * ConstructorsCrud$ModuleRequireFormat = @"(function(exports){"
...
@implementation ConstructorsCrudObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_constructorscrud);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ConstructorsCrud global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ConstructorsCrud" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_constructorscrud = [[ConstructorsCrudObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_constructorscrud];
	
	NSString *_constructorscrudNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _constructorscrudNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_constructorscrud valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ConstructorsCrud$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.constructorscrud.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.constructorscrud.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
